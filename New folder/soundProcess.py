...
def process_audio(data):
	global current_silence, audio_bank, RecNumber, startTime
	should_record = False
	...
	if should_record == False and len(audio_bank) > 0:
		...
		##take snapshot
		subprocess.Popen(['curl','-s','-o','/dev/null','http://' + ip + ':8080/0/action/snapshot'], stdout=FNULL, stderr=subprocess.STDOUT)
		##removal silence and split up the wav file
		subprocess.Popen(['python','silenceRemoval.py', str(filename), str(RecNumber), str(startTime), str(int(time.time()))], stdout=FNULL, stderr=subprocess.STDOUT)
		print(str(startTime))
